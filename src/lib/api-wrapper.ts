...
export async function submitFollowupQuestion(
  ...
  if (config.useLangChain && config.enableSemanticSearch && analysisData) {
    ...
    const apiKey = localStorage.getItem('clause-clarity-api-key') || '';
    return await submitEnhancedFollowupQuestion(question, originalClause, analysisData, apiKey);
